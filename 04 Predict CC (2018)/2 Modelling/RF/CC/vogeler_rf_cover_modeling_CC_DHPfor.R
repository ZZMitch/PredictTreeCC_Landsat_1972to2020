...
data = read.csv(file="C:/Users/bonneymi/Google Drive/PHD/Thesis Research/Landsat Time-Series/Validate/Ground Validation/LandsatVI_DHP_2018.csv", header=TRUE, sep=",")
attach(data)
names(data)
...
rfsel_1 = rf.modelSel(data[,c(3:22)], # change these column numbers to those that correspond to all of the predictors in the data file (so in this example, my predictors were in rows 14-27 in my data csv)
                      as.numeric(data[,"pCC_1"]), #or whatever your cover proportion column is called
                      r = c(seq(1:20)/(21)), # change this to the number of predictors you have included in your data set (in the defined column range above)
...
rf.fit1 = randomForest(cover ~ TCA_mn, ntree = ntree, 
                       data = data, importance  = TRUE)
...
test = data.frame(matrix(NA, nrow = 28, ncol = 0))
...
cover_base = paste0(BASEPATH,'cc_dhp/') # change this to the output path for predicted cover maps
...
xlist =  list(NDVI_mn = NDVI_mn, TCA_mn = TCA_mn) 
xtypes = list(NDVI_mn = "numeric", TCA_mn = "numeric")

# output asc predictive map grid file path
output_file = paste0(cover_base,'ccdhp_for_',YEAR,'.asc')

...
# %CC
set.seed(rf_seed)
cover = as.numeric(data$pVC_1) # cover as a proportion
...
cover_rf = randomForest(cover ~ NDVI_mn + TCB_mn + TCG_mn + TCW_mn + TCA_mn + NDVI_sd + TCB_sd + TCG_sd + 
                          ...
                          Slope_sd + Aspect_sd + SSINA_sd + SCOSA_sd + LC_num + CC_10, ntree = ntree, 
...
rfsel_1 = rf.modelSel(data[,(5:26)], # change these column numbers to those that correspond to all of the predictors in the data file (so in this example, my predictors were in rows 14-27 in my data csv)
                      as.numeric(data[,"pVC_1"]), #or whatever your cover proportion column is called
                      r = c(seq(1:22)/(22)), # change this 14 to the number of predictors you have included in your data set (in the defined column range above)
...
rf.fit1 = randomForest(cover ~ NDVI_mn + TCG_mn + TCW_mn + TCA_mn + NDVI_sd + LC_num, ntree = ntree, 
                       data = data, importance  = TRUE)
...
# OOB - test
summary(lm(data$pVC_1 ~ rf.fit1$predicted)) # R2 approx. equal to RF pve

test = data.frame(matrix(NA, nrow = nrow(data), ncol = 0))
test$pVC = data$pVC_1 * 100
test$predicted = rf.fit1$predicted * 100
test$LandCover = data$LandCover
write.csv(test, "VC_VHR_obspred.csv")

train.control = trainControl(method = "LOOCV")
train(pVC ~ predicted, data = test, method = "lm", trControl = train.control)

plot(test$predicted, test$pVC) # predicted based on OOB samples...
abline(0,1, lwd = 3)
abline(lm(data$pVC_1 ~ rf.fit1$predicted), lty = 3, lwd = 3, col = "red")
...
cover_base = paste0(BASEPATH,'vc_vhr/') # change this to the output path for predicted cover maps
...
xlist =  list(NDVI_mn = NDVI_mn, TCG_mn = TCG_mn, TCW_mn = TCW_mn, TCA_mn = TCA_mn, NDVI_sd = NDVI_sd, 
              LC_num = LC_num) 
xtypes = list(NDVI_mn = "numeric", TCG_mn = "numeric", TCW_mn = "numeric", TCA_mn = "numeric", 
              NDVI_sd = "numeric", LC_num = "character")

# output asc predictive map grid file path
output_file = paste0(cover_base,'vcvhr_',YEAR,'.asc')

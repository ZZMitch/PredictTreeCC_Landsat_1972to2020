...
data = subset(data, LandCover == "Residential")
...
rf.fit1 = randomForest(cover ~ NDVI_mn + TCG_mn + TCW_mn + TCA_mn + Elev_mn + CC_10, ntree = ntree, 
                       data = data, importance  = TRUE)
...
test = data.frame(matrix(NA, nrow = 126, ncol = 0))
...
cover_base = paste0(BASEPATH,'cc_vhr/residential/') # change this to the output path for predicted cover maps
...
Elev_mn = (paste0(asc_base,'elev_mn.asc'))
CC_10 = (paste0(asc_base,'cc10_',YEAR,'.asc'))

xlist =  list(NDVI_mn = NDVI_mn, TCG_mn = TCG_mn, TCW_mn = TCW_mn, TCA_mn = TCA_mn, Elev_mn = Elev_mn,
              CC_10 = CC_10) 
xtypes = list(NDVI_mn = "numeric", TCG_mn = "numeric", TCW_mn = "numeric", TCA_mn = "numeric", Elev_mn = "numeric",
              CC_10 = "character")

# output asc predictive map grid file path
output_file = paste0(cover_base,'ccvhr_res_',YEAR,'.asc')

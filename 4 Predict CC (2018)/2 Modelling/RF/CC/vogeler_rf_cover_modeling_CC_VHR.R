...
cover_rf = randomForest(cover ~ NDVI_mn + TCB_mn + TCG_mn + TCW_mn + TCA_mn + NDVI_sd + TCB_sd + TCG_sd + 
                          ...
                          data = data, importance  = TRUE)  #mtry = 
cover_rf
...
rfsel_1 = rf.modelSel(data[,(5:25)], # change these column numbers to those that correspond to all of the predictors in the data file (so in this example, my predictors were in rows 14-27 in my data csv)
...
cover_base = paste0(BASEPATH,'cc_vhr/') # change this to the output path for predicted cover maps
...
output_file = paste0(cover_base,'ccvhr_',YEAR,'.asc')

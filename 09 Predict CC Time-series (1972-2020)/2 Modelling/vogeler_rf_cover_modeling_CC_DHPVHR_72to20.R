...
data_vhr = read.csv(file="C:/Users/mitch/Google Drive/PHD/Thesis Research/Landsat Time-Series/Validate/CC Timeseries/LandsatVI_VHR_2018.csv", header=TRUE, sep=",")
attach(data_vhr)
names(data_vhr)

data_vhr = subset(data_vhr, Include == 1) # Remove bad sites
data_vhr = subset(data_vhr, select = -c(Include)) # Remove unneeded columns

# DHP sites
data_dhp = read.csv(file = "C:/Users/mitch/Google Drive/PHD/Thesis Research/Landsat Time-Series/Validate/CC Timeseries/LandsatVI_DHP_2018.csv", header=TRUE, sep=",")
data_dhp$LandCover2018 = "Forest"
...
#data_vhr = subset(data_vhr, pCC < 75 | LandCover2018 != "Forest") # Remove VHR forest sites with %CC 75 or more
...
#data_vhr = subset(data_vhr, pCC < 85 | LandCover2018 != "Forest") # Remove VHR forest sites with %CC 85 or more, , fills early gap in DHP model
...
#data_vhr = subset(data_vhr,  LandCover2018 != "Forest")

#data_vhr = subset(data_vhr, pCC < 85 | pCC > 95 | LandCover2018 != "Forest")
...
vars
# Without CC_10: TCA_mn, TCB_mn, TCG_mn, TCW_mn, TCA_sd

set.seed(rf_seed) # make sure to set the seed again prior to each time you run a model
# Insert the final selected variables from the selvars above
rf.fit1 = randomForest(cover ~ TCA_mn + TCB_mn + TCG_mn + TCW_mn + TCA_sd + CC_10, ntree = ntree, 
                       data = data, importance  = TRUE)
...
# Set some base paths
BASEPATH = "C:/Users/mitch/Google Drive/PHD/Thesis Research/Landsat Time-Series/Validate/CC Timeseries/Data/" # set a base path here for your processing
...
#TCA_mn = (paste0(BASEPATH,'tca_',YEAR,'.asc'))
TCB_mn = (paste0(BASEPATH,'tcb_mn_',YEAR,'.asc'))
#TCB_mn = (paste0(BASEPATH,'tcb_',YEAR,'.asc'))
TCG_mn = (paste0(BASEPATH,'tcg_mn_',YEAR,'.asc'))
#TCG_mn = (paste0(BASEPATH,'tcg_',YEAR,'.asc'))
TCW_mn = (paste0(BASEPATH,'tcw_mn_',YEAR,'.asc'))
#TCW_mn = (paste0(BASEPATH,'tcw_',YEAR,'.asc'))
TCA_sd = (paste0(BASEPATH,'tca_sd_',YEAR,'.asc'))
CC_10 = (paste0(BASEPATH,'cc10_',YEAR,'.asc'))
#CC_10 = (paste0(BASEPATH,'cc10_nomn_',YEAR,'.asc'))

xlist =  list(TCA_mn = TCA_mn, TCB_mn = TCB_mn, TCG_mn = TCG_mn, TCW_mn = TCW_mn, TCA_sd = TCA_sd, 
              CC_10 = CC_10) 
xtypes = list(TCA_mn = "numeric", TCB_mn = "numeric", TCG_mn = "numeric", TCW_mn = "numeric", 
              TCA_sd = "numeric", CC_10 = "character") 

# output asc predictive map grid file path
output_file = paste0(BASEPATH,'cc_nomn_',YEAR,'.asc')
